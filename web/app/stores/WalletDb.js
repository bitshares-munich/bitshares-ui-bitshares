...
import {ChainStore, PrivateKey, key, Aes, hash} from "bitsharesjs/es";
...
class WalletDb extends BaseStore {
    ...
    constructor() {
        ...
        this._export(
            ...
            "importKeysWorker", "create_stealth_wallet", "create_new_stealth_account", "add_stealth_contact", "get_my_stealth_accounts", "get_my_stealth_contacts"
        );
    }
    ...
    //Stealth stuff here////////Note: This is a test version, not to be used in production version!////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //Websql having some issues I'm forced to use 'too long' functions for now, will find  a workaround afterwards
    add_stealth_contact(contact)
    {
        console.log("ADDING CONTACT "+contact.label+" ~ "+contact.publickey);
        let db = openDatabase("Stealth_Wallet", "1.0", "Stealth Keys", 2 * 1024 * 1024);
        if(contact.label != "" && contact.label != null && contact.publickey != null && contact.publickey != "")
        {
            console.log("CHECKING TO SEE IF EXISTS");
            db.transaction(function (tx) 
            {
                tx.executeSql("SELECT * FROM stealth_labels WHERE label = ?", [contact.label], function(tx, results) 
                {
                    if(results.rows.length < 1) 
                    {
                        ...
                        db.transaction(function (tx) 
                        {
                            tx.executeSql("INSERT INTO stealth_labels (label, pubkey) VALUES (?, ?)",[contact.label, contact.publickey]);
                        });
                    ...
                    {
                        throw new Error("ERROR, LABEL ALREADY EXISTS!");
                    }
                });
            });
        }
        ...
        {
            throw new Error("add_stealth_label: ERROR!");
        }

    }
    //Return a list of stealth accounts
    get_my_stealth_accounts()
    {
            ...
            db.transaction(function (tx) 
            {
                tx.executeSql("SELECT * FROM stealth_accounts", [], function(tx, results) 
                {
                    for(var i=0;i<results.rows.length;i++)
                    {
                        ...
                        console.log("Wonder if it worked:\n"+stealth_accounts[0].label);
                    }
    ...
    //label, brainkey, associated_account
    create_new_stealth_account(ACC)
    {
        var db = openDatabase("Stealth_Wallet", "1.0", "Stealth Keys", 2 * 1024 * 1024);
        if(ACC.label !== "" && ACC.label !== null && ACC.brainkey !== null && ACC.brainkey !== "")
        {
            db.transaction(function (tx) 
            {
                tx.executeSql("SELECT * FROM stealth_accounts WHERE label = ?", [ACC.label], function(tx, results) 
                {
                    if(results.rows.length < 1) 
                    {
                        db.transaction(function (tx) 
                    ...
                    {
                        console.log("ERROR, STEALTH ACCOUNT ALREADY EXISTS!");
                    }
                });
            });
        }
        ...
        {
            console.log("Invalid input, brainkey or label are empty");
        }
    }
